  ...
  setSamplesAudioBuffer(data, canvasArea, colormap) {
    ...
    return new Promise(resolve => {
      that.audioContext.decodeAudioData(data, function(buff) {
        ...
        if (that.channels.length > 2) {
          width /= 2;
        }

        for (let i = 0; i < that.channels.length/2; i++) {
          let folder = that.menu.addFolder("Image " + i);
          that.folders.push(folder);
          let canvas = document.createElement("canvas");
          canvas.width=width;
          canvas.height=width;
          canvasArea.appendChild(canvas);
          let stippleCanvas = new StippleCanvas(canvas, that.audioPlayer, that.channels[i*2], that.channels[i*2+1], that.sr, folder, colormap);
          if (i%2 == 1) {
            canvasArea.appendChild(document.createElement("p"));
          }
        }
