...
class StippleCanvas {
     ...
     * @param {Float32Array} x Channel 1 of audio samples
     * @param {Float32Array} y Channel 2 of audio samples
     ...
     */
    constructor(glcanvas, audioPlayer, x, y, sr, folder, colormap) {
    ...
    setupBuffers() {
        ...
        for (let i = 0; i < N; i++) {
            x[i] -= 0.5*(mins[0] + maxes[0]);
            y[i] -= 0.5*(mins[1] + maxes[1]);
        }
    ...
    render() {
        ...
        let N = Math.floor(this.audioPlayer.currentTime*this.sr);
