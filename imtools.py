...
def normalize_to_square(X, scale=2):
    """
    Normalize the points to a [-scale/2, scale/2]^2 square
    ...
    X: ndarray(N, 2)
        Points to normalize
    scale: float
        Scale to which to normalize them
    
    Returns
    -------
    ndarray(N, 2): The normalized points
    """
    X = X - np.min(X, axis=0)[None, :]
    X = X*scale/np.max(X)
    xmin = np.min(X, axis=0)
    xmax = np.max(X, axis=0)
    X -= 0.5*(xmin+xmax)[None, :]
    return X
